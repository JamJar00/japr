...
def check_directory(directory, project_type, is_summary=False, is_profile=False):
    ...
    if is_profile:
        print("Profile mode is enabled. Showing all checks performed, not just failed ones")

    for (result, check, profile_time) in issues:
        if (result.result == Result.FAILED and result.id not in ignored_checks) or is_profile:
            # Build up components then display for code clarity
            emoji_block = "\N{cross mark}" if result.result == Result.FAILED else "\N{heavy check mark}" if result.result == Result.PASSED else "\N{white question mark ornament}"
            ...
            file_block = f"[{result.file_path}] " if result.file_path is not None else ""

            if is_summary:
                print(f"{severity_color}{check.severity.name.ljust(6)}\033[0;0m - \033[1m{check.id}\033[0;0m {file_block}{check.reason}")
            elif is_profile:
                print(f"{emoji_block} {severity_color}{check.severity.name.ljust(6)}\033[0;0m - {profile_block} - \033[1m{check.id}\033[0;0m {file_block}{check.reason}")
            else:
                print(f"{severity_color}{check.severity.name.ljust(6)}\033[0;0m - \033[1m{check.id}\033[0;0m {file_block}{check.reason}")
                print()
                print(check.advice)
    ...
    score = int(5 - sum(check.severity.value for (result, check, _) in issues if result.result == Result.FAILED or result.result == Result.PRE_REQUISITE_CHECK_FAILED) / sum(check.severity.value for (_, check, _) in issues) * 5)
    print("\033[1mProject score: " + "\U00002B50" * score + "\033[0;0m (out of 5)")

    passed = len([result for (result, _, _) in issues if result.result == Result.PASSED and result.id not in ignored_checks])
    failed = len([result for (result, _, _) in issues if result.result == Result.FAILED and result.id not in ignored_checks])
    cannot_run = len([result for (result, _, _) in issues if result.result == Result.PRE_REQUISITE_CHECK_FAILED and result.id not in ignored_checks])
    suppressed = len([result for (result, _, _) in issues if result.id in ignored_checks])
    ...
    return len([result for (result, _, _) in issues if result.result == Result.FAILED and result.id not in ignored_checks]) == 0
...
group.add_argument('--profile', help="times how long each check takes to run", action='store_true')
group.add_argument('-s', '--summary', help="prints results in summary form", action='store_true')
